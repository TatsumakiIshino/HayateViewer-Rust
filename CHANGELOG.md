...
- **設定画面（Modern UI）へのアニメーション設定の統合**
  - レンダリングタブからアニメーションの有効/無効、速度、種類（スライド/カール）を直接変更可能に。
  - 矢印キーおよびクリック操作に完全対応。
- **D3D11 ページカールアニメーションの抜本的改善**
  - 背表紙（Spine）を軸とした回転モデルを採用し、本をめくるような滑らかで自然な動きを実現。
  - めくられるページの「裏面」に、次のページの画像を正しく表示する機能を実装（ミラー表示や白紙の解消）。
  - 見開き表示時に、複数のページにまたがって連続した一つのカールとして描写されるようロジックを統合。
  - シェーダーを更新し、表面・裏面のピクセル判定とテクスチャ反転処理を追加。
