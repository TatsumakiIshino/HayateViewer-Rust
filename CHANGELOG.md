...
## [0.6.7-test] - 2026-05-16
### 追加
- **AVIF形式のサポート**:
  - 画像読み込みライブラリの `avif` 機能を有効化し、単体ファイルおよび書庫ファイル内の `.avif` 画像の表示に対応しました。

### 修正
- **見開き表示時のページ送りオフセット保持バグの修正**:
  - 見開き表示中に1ページ分ずらした状態（オフセット状態）でページ送りや「Shift + 矢印キー」での10ページスキップを行った際、ずらし状態がリセットされてしまう不具合を修正しました。今後は手動でずらしたページ配置の偶数・奇数ペアを維持したまま、スムーズに前後へのページ送りが可能になります。

- **D3D11 レンダラの安定性と色再現の改善**:
  - `compile_shader`: `windows::core::Error::new` を使用して、シェーダーコンパイルエラー時の `HRESULT` 型不整合によるビルドエラーを修正。
  - `DXGI_SWAP_CHAIN_DESC1`: スワップチェーンのフォーマットを `DXGI_FORMAT_B8G8R8A8_UNORM` に変更し、一部環境での初期化エラーを回避。
  - `create_rgba_texture`: テクスチャフォーマットを `DXGI_FORMAT_R8G8B8A8_UNORM` に変更し、RGBA データの並びと一致させることで正しい色で表示されるように修正。

## [0.6.5-test] - 2025-12-28
### 追加
- **詳細なキャッシュ設定機能の実装**:
  - オプション画面（全般タブ）に「CPUキャッシュ容量」「先読みページ数 (CPU)」「先読みページ数 (GPU)」の設定項目を追加。
  - キャッシュ容量は 512MB 単位、先読みページ数は 5 ページ単位で動的に変更可能。
  - 設定変更は即座にメモリ管理および先読み処理に反映されます。

### 修正
- **CPUキャッシュ破棄ロジックの改善**:
  - 指定されたキャッシュ容量を超えた際、古いキャッシュ（LRU: Least Recently Used）から確実に破棄されるようにロジックを修正。これにより、長時間使用時に新しいページが読み込めなくなる問題を解決しました。
- **ImageCache の拡張**:
  - 実行中にキャッシュ容量の最大値を動的に変更できる `set_max_bytes` メソッドを追加。

## [0.6.4-test] - 2025-12-27
### 修正
- `D3D11` レンダラにおいて、RGBA テクスチャおよびスワップチェーンのフォーマットを `_SRGB` に変更しました。これにより、sRGB 画像が正しいガンマ補正で表示され、色合いがおかしくなる問題が解消されました。
- `D3D11` レンダラの YCbCr 定数（オフセット・スケーリング）を、8-bit 整数データおよび BT.709 行列と整合するように再調整しました。

## [0.6.3-test] - 2025-12-27

### 追加

- **ヘルプ画面 (Hキー) の実装**: キーボードショートカット一覧を表示するオーバーレイウィンドウを追加しました。
  - ウィンドウサイズや描画の調整を行い、すべてのキーバインドが正しく表示されるように改善しました。
  - 'H'キーで開く機能のみを実装し、他のウィンドウとのイベント競合を解消しました。

### 修正

- **キー入力処理**: ヘルプウィンドウ開閉時のイベント伝播の不整合を解消し、安定した動作を実現しました。

### 変更

- **'B' キーによる表示モード変更をオプション画面にリアルタイム反映**
  - オプション画面（Modern UI）を表示中に 'B' キーで表示モードを切り替えた際、オプション画面内の該当項目も即座に更新されるように修正。
  - 'B' キー押下時に設定ファイル (`config.json`) への自動保存が行われるように改善。
